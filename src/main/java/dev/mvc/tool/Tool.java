...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
  ...
  public static String inquiryBox(int now_page, String word_title, String word_name, int state, String list_file_name, int search_count, 
                                      int record_per_page, int page_per_block){    
    // 전체 페이지 수: (double)1/10 -> 0.1 -> 1 페이지, (double)12/10 -> 1.2 페이지 -> 2 페이지
    int total_page = (int)(Math.ceil((double)search_count / record_per_page));
    // 전체 그룹  수: (double)1/10 -> 0.1 -> 1 그룹, (double)12/10 -> 1.2 그룹-> 2 그룹
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    int _now_page = (now_grp - 1) * page_per_block;  
    if (now_grp >= 2){ // 현재 그룹번호가 2이상이면 페이지수가 11페이지 이상임으로 이전 그룹으로 갈수 있는 링크 생성 
      str.append("<span class='span_box_1'><A href='"+list_file_name+"?&word_title="+word_title+"&word_name="+word_name+"&state="+state+"&now_page="+_now_page+"'>이전</A></span>"); 
    } 
    ...
    for(int i=start_page; i<=end_page; i++){ 
      ...
      if (now_page == i){ // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
        ...
        // 현재 페이지가 아닌 페이지는 이동이 가능하도록 링크를 설정
        str.append("<span class='span_box_1'><A href='"+list_file_name+"?&word_title="+word_title+"&word_name="+word_name+"&state="+state+"&now_page="+i+"'>"+i+"</A></span>");   
      } 
    } 
    ...
    _now_page = (now_grp * page_per_block)+1; //  최대 페이지수 + 1 
    if (now_grp < total_grp){ 
      str.append("<span class='span_box_1'><A href='"+list_file_name+"?&word_title="+word_title+"&word_name="+word_name+"&state="+state+"&now_page="+_now_page+"'>다음</A></span>"); 
    } 
    str.append("</div>"); 
